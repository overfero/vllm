...
# across exactly 3 machines - PP=3 + EP=2 CANDIDATE (2026-08-15 benchmark),
# NOT the baseline. Identical topology/checkpoints/transport to
# cluster/qwen35_122ba10b_3machine.sh (TP=2/PP=3, custom UDP hole-punch
# transport, NO MTP, NO Machine D, 48 layers split 16/16/16) - the ONLY
# difference is `--enable-expert-parallel` on all 3 stages, which reuses
# each stage's already-local TP=2 torch.distributed group (see
# scripts/stage_server.py / scripts/launch_pp_stage.py's matching flag
# docstrings) to shard MoE's 256 routed experts whole-expert-per-GPU
# (128/128) instead of splitting every expert's matrices in half. This does
# NOT touch the cross-machine PP transport at all.
#
# Baseline (PP3+TP2) and this candidate (PP3+EP2) CANNOT run at the same
# time - both want ~14GiB/GPU on the same 6 T4s. This script kills any
# running baseline (or a stale candidate run) before launching, same as the
# baseline script's own cleanup step.
#
# Usage (run from the repo root, .env already filled in - shared with the
# baseline script, no separate credentials needed):
#   ./cluster/qwen35_122ba10b_3machine_ep.sh
#
...
# Idempotent for the install/checkpoint steps; always kills+relaunches the
# serving processes.

set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/.."

# Separate lockfile from the baseline script - this prevents two concurrent
# invocations of THIS script, not concurrent baseline+candidate (those are
# mutually exclusive by GPU capacity, not by lock; the cleanup step below
# handles that by killing whichever one is currently running).
LOCKFILE="/tmp/qwen35_122ba10b_3machine_ep.lock"
exec 200>"$LOCKFILE"
if ! flock -n 200; then
  echo "[3machine-ep] ERROR: another instance of this script is already running (lock: $LOCKFILE) - refusing to start a second one." >&2
  exit 1
fi
...
# Same env/dep checks as the baseline script - idempotent no-ops if the
# baseline run already set this box up (expected case for an A/B test).
...
curl -s -m 10 "$SIGNALING_URL/docs" -o /dev/null -w "[3machine-ep] public signaling URL check: HTTP %{http_code}\n" || true
...
#      run) - PP3+TP2 and PP3+EP2 cannot coexist on the same 6 GPUs. ----
log "=== stopping any running deployment (baseline or stale candidate) before launching PP3+EP2 ==="
...
# ---- 5. launch all 3 stages together, WITH --enable-expert-parallel ----
log "=== launching PP3+EP2 candidate (CUDA graph, no MTP) ==="
rm -f deploy_A_ep.log
nohup python3 -u scripts/stage_server.py \
  ...
  --max-num-seqs $MAX_NUM_SEQS --enable-cudagraph --enable-expert-parallel > deploy_A_ep.log 2>&1 &
...
sshpass -p "$MACHINE_B_PASSWORD" ssh -o StrictHostKeyChecking=no -p "$MACHINE_B_PORT" root@127.0.0.1 \
  ...
  --quantization gptq --dtype float16 --language-model-only --max-model-len $MAX_MODEL_LEN \
  --gpu-memory-utilization $GPU_MEM_UTIL --num-gpu-blocks-override $NUM_GPU_BLOCKS_OVERRIDE \
  --max-num-seqs $MAX_NUM_SEQS --enable-cudagraph --enable-expert-parallel \
  > /kaggle/working/vllm/deploy_B_ep.log 2>&1 < /dev/null & disown; echo launched" &
...
sshpass -p "$MACHINE_C_PASSWORD" ssh -o StrictHostKeyChecking=no -p "$MACHINE_C_PORT" root@127.0.0.1 \
  ...
  > /kaggle/working/vllm/deploy_C_ep.log 2>&1 < /dev/null & disown; echo launched" &
...
for i in $(seq 1 40); do
  if sshpass -p "$MACHINE_C_PASSWORD" ssh -o StrictHostKeyChecking=no -p "$MACHINE_C_PORT" root@127.0.0.1 \
      "grep -q 'Application startup complete' /kaggle/working/vllm/deploy_C_ep.log 2>/dev/null"; then
    READY=1
    break
  fi
  if sshpass -p "$MACHINE_C_PASSWORD" ssh -o StrictHostKeyChecking=no -p "$MACHINE_C_PORT" root@127.0.0.1 \
      "grep -qi 'Traceback' /kaggle/working/vllm/deploy_C_ep.log 2>/dev/null"; then
    log "ERROR: Machine C crashed during startup - check deploy_C_ep.log on that machine"
    exit 1
  fi
  sleep 20
done
...
if [[ "$READY" -ne 1 ]]; then
  log "ERROR: timed out waiting for Machine C's API server - check deploy_A_ep.log locally and deploy_B_ep.log/deploy_C_ep.log on their machines"
  exit 1
fi

sshpass -p "$MACHINE_C_PASSWORD" ssh -o StrictHostKeyChecking=no -p "$MACHINE_C_PORT" root@127.0.0.1 \
  "curl -s http://127.0.0.1:${DRIVER_PORT}/health -o /dev/null -w '[3machine-ep] API health check: HTTP %{http_code}\n'"

log "=== PP3+EP2 CANDIDATE DEPLOYMENT READY ==="
