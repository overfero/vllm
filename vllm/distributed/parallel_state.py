...
if TYPE_CHECKING:
    ...
    from vllm.transport import Transport
...
def patched_fused_scaled_matmul_reduce_scatter_fake(
    ...
    group_name: str,
    output_shape: List[int],  # noqa: UP006 - torch<2.9's infer_schema doesn't accept PEP 585 `list[int]` here (see TODO above), but does accept typing.List[int] for the same type
...
class GroupCoordinator:
    ...
    # Opt-in override: when set, .send()/.recv() route a single point-to-point
    # tensor through TransportProcessGroup (vllm/transport/) instead of
    # device_communicator/NCCL. Class-level default so subclasses that build
    # instances without calling this __init__ (e.g. StatelessGroupCoordinator)
    # still safely default to the existing NCCL/gloo path. See
    # vllm/transport/README.md ("Phase 3") for scope.
    transport: "Transport | None" = None
    # A transport-backed PP group of exactly 2 members (one first rank, one
    # last rank) only ever needs one peer, so `transport` alone is enough -
    # that's the only shape the original Phase 5 proof
    # (test18_real_bootstrap_pp.py) exercised. A pipeline of 3+ stages has
    # middle ranks with two distinct peers (the previous stage and the next
    # one), which a single `transport` attribute can't address: `send()`
    # always needs to go to the *next* rank and `recv()` always needs to
    # come from the *previous* rank, and for a middle rank those are two
    # different connections. `transport_prev`/`transport_next` are additive,
    # optional overrides for that case - when unset (the 2-member case),
    # every method below falls back to the single `transport` attribute
    # exactly as before, so nothing about the already-proven 2-stage path
    # changes. See `install_transport_pp_group` in
    # vllm/transport/pipeline_bootstrap.py for how these get populated.
    ...
    def recv_tensor_dict(
        ...
        """
        if self.transport is not None:
            return _transport_recv_tensor_dict(self.transport_prev or self.transport)
        # Bypass the function if we are using only 1 GPU.
        if not torch.distributed.is_initialized() or self.world_size == 1:
            return None
    ...
    def send(self, tensor: torch.Tensor, dst: int | None = None) -> None:
        ...
        if self.transport is not None:
            ...
            TransportProcessGroup(self.transport_next or self.transport).send_tensor(tensor)
            return
        if self.device_communicator is None:
            raise ValueError("No device communicator found")
    ...
    def recv(
        ...
        if self.transport is not None:
            ...
            tensor, _stats = TransportProcessGroup(self.transport_prev or self.transport).recv_tensor()
            return tensor
        if self.device_communicator is None:
            raise ValueError("No device communicator found")
