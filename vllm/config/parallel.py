...
TransportBackend = Literal["tcp", "udp"]
...
class ParallelConfig:
    ...
    """Worker-to-worker communication transport. "tcp" is the default plain-
    socket transport; "udp" routes through vllm.transport.UDPTransport (a
    NAT hole-punched UDP transport). Selection happens in exactly one place:
    vllm.transport.get_transport(). Overridable via the VLLM_TRANSPORT
    environment variable. As of this phase, this field is plumbed through
    config but not yet consumed by the real tensor/pipeline-parallel data
    plane (NCCL/gloo) - see vllm/transport/README.md."""
